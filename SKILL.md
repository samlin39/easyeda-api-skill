...
7. **Bridge service not found despite EasyEDA "Reconnect"**: Check for a stale macOS LaunchAgent or daemon that starts `scripts/bridge-server.mjs` directly from a runtime skill folder such as `/Users/yodo/.codex/skills/easyeda-api-skill` or from the source checkout. Symptoms include `ERR_MODULE_NOT_FOUND: Cannot find package 'ws'`, repeated launchd restarts, or `/Users/yodo/bin/easyeda-bridge: line 21: exec: node: not found`. Fix the service to run `/Users/yodo/bin/easyeda-bridge` with working directory `/Users/yodo/work/easyeda-api-runtime`, and ensure launchd has a PATH containing the Node.js binary, for example `/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin`.
